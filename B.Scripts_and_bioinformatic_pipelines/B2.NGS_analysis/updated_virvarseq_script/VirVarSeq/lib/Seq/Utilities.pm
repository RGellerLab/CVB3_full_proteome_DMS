...
#### Edited by C.Toft on the 8t March 2023
## The program can now identify deleted codons and incorporate them into the codon analysis. It will only use reads which have a single codon deletion.  
...
sub sam2CodonTable {
	...
	my($count1,$count2,$count3,$countN) = (0,0,0,0);
	my(@array);
	...
	while ($line = <IN>){  
		...
		#####   CT addition
		########################################################
		#print "$match " . length($read). "\n$read\n";
		if ($match =~ m/[ID]/) {
			
			# 10H10M3D103M10H
			if($match =~ m/(\d+)H(\d+)M3D(\d+)M(\d+)H/){ # identify reads with a single codon deletion. Add info to read and quality, as well as edition the cigar
				my $insert =  $2 + $pos - $region_start;
				my $test = $insert % 3;
				my $test2 = uc substr $ref, ($pos - 1), ($2 + $3 + 3);
				my $test3 = uc substr $ref, $region_start -1, $region_len ;
				#print "$line$read\n$test2\n$match regionStart=$region_start refStart=$ref_start  $1 $2 pos=$pos startOnRef=$insert $test\n";
				my $min = $2;
				$min = $3 if $3 < $2;
				push(@array,$min);
				if(($insert) % 3 == 0){
					#print "$read\n$match >$1< $pos $insert $test\n";
					#$read =~ s/(.{$1})($2)/$1\NNN$2/;
					...
					my($x) = $2+$3+3 ;
					$match =~ s/\d+M3D\d+/$x/;
					#print "In frame: $match\n$read\n$test2\n\n" if $seq =~ /ACCTTATGGCTGAAATATATTGAC#ab/;

					#die;
					$count1++;
				}else{
					
					my $hanging = substr($read, ($2 - $test), 3); 
					my $refM1 = uc substr($ref, $2 + $pos - 1 - $test, 3);
					my $refM2 = uc substr($ref, $2 + $pos - 1 - $test + 3, 3);

					if($hanging eq $refM1 || $hanging eq $refM2){

						if($hanging eq $refM2){
							substr($read,$2-$test,0,"---");
							substr($qual,$2-$test,0,"JJJ");		
						}elsif($hanging eq $refM1){
							substr($read,$2+3-$test,0,"---");
							substr($qual,$2+3-$test,0,"JJJ");
						}else{
							#print "problems\n";
							next;
						}					
						...
						$match =~ s/\d+M3D\d+/$x/;
						#print "$read\n$test2\n\n";
						#print "Correct frame: $match $test hanging=$hanging rf1=$refM1 ref2=$refM2\n$read\n$test2\n\n" if $seq =~ /ACCTTATGGCTGAAATATATTGAC#ab/;
						#print "$line$read\n$test2\n$match regionStart=$region_start refStart=$ref_start  $1 $2 pos=$pos startOnRef=$insert $test hanging=$hanging rf1=$refM1 ref2=$refM2\n";
						#die;
					}elsif($2 < 3){ # when the codon deletion is at the beginning of the read.

						$hanging = substr($read,$test, 3);
						$refM1 = uc substr($test2,$test + 3, 3);

						$count2++;

						#next if $count2 > 20;
						#print "$line$read\n$test2\n$match regionStart=$region_start refStart=$ref_start  $1 $2 pos=$pos startOnRef=$insert lengthHanging=$test hanging=$hanging rf1=$refM1 ref2=$refM2\n";
						
						if($2 == 2 && $test == 2){
							$hanging = substr($read,3, 3);
							$refM1 = uc substr($test2,6 , 3);
							if($hanging eq $refM1){
								substr($read,3,0,"---");
								substr($qual,3,0,"JJJ");
								my($x) = $2+$3+3 ;
...
#print "4 @nt_arr\n"; 
   ...
   #print "5 @nt_arr\n$codon_len\n"; 	
	...
	#print "indel=$count1 wrongAligned=$count2 failed=$count3 N-NextCodon=$countN\n";
	$" = "\n";
	#print "@array";
	...
	foreach my $pos (sort {$a <=> $b} keys %codon_table){
		...
		foreach my $codon (sort keys %codons){   				
			...
			if (($cnts{'forward'} > 0) || ($cnts{'reverse'} > 0)){
				...
				#print "$ref_aa ($ref_codon) $aa ($codon)\n" if $ref_aa ne $aa;
